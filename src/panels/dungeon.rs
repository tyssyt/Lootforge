...
use crate::{combat::{battle::Battle, combatant::Combatant}, dungeon::dungeon::DungeonData, timekeeper::FrameInfo};

use super::{animation::Animation, rewards::{add_chest_button, RewardsWindow}};
...
impl DungeonPanel {
    pub fn show(&mut self, ui: &mut Ui, dungeon: &mut DungeonData, wardrobe: &Wardrobe, rewards_window: &mut RewardsWindow, frame_info: FrameInfo) {
        ui.horizontal(|ui| {
            ui.heading("Dungeon");
            ...
            ui.with_layout(Layout::right_to_left(Align::BOTTOM), |ui| {
                ...
                if ui.checkbox(&mut dungeon.auto_restart, "auto restart").clicked() && dungeon.auto_restart && dungeon.cur.finished {
...
fn show_battle(ui: &mut Ui, rect: Rect, battle: &Battle, battle_data: &BattleData, transition: Option<f32>) {
    ...
    for ((offset, enemy), enemy_data) in ENEMY_OFFSETS[battle.enemies.len()].iter().zip(battle.enemies.iter()).zip(battle_data.enemies.iter()) {
        if !enemy.alive() {
            continue;
        }
...
const ENEMY_OFFSETS: [&[Vec2]; 5] = [
    ...
    &[vec2(-30. + Background::SIZE.x / 4., -15.), vec2(-30. +Background::SIZE.x / 4., 90.), vec2( 70. +Background::SIZE.x / 4., 40.)],
    &[vec2(-30. + Background::SIZE.x / 4., -15.), vec2(-30. +Background::SIZE.x / 4., 90.), vec2( 70. +Background::SIZE.x / 4., -15.), vec2( 70. +Background::SIZE.x / 4., 90.)],
];
...
fn show_health_bar(ui: &mut Ui, char: &Combatant, char_stats: &CharStats) {
    ...
    if percent_health + percent_shield > 1. {
        ...
        let shield_rect = Rect::from_min_max( allocated_rec.min + vec2(allocated_rec.width() * (1. - percent_shield), 0.), allocated_rec.max);
        ...
        let shield_rect = Rect::from_min_size(allocated_rec.min, vec2(allocated_rec.width() * (percent_shield + percent_health), allocated_rec.height()));
        ui.painter().rect_filled(shield_rect, CornerRadius::ZERO , Color32::LIGHT_GRAY);
        let health_rect = Rect::from_min_size(allocated_rec.min, vec2(allocated_rec.width() * percent_health, allocated_rec.height()));
...
fn show_tooltip(ui: &mut Ui, char: &Combatant, char_stats: &CharStats) {
    ...
    ui.label(health);
    ui.label(format!("Resistances: {}/{}/{}/{}", char_stats.resistances.bleed, char_stats.resistances.fracture, char_stats.resistances.madness, char_stats.resistances.void));
    ui.label(format!("Heal Power: {}%, Shield Power: {}%", char_stats.heal_power*100., char_stats.shield_power*100.));
...
#[derive(Clone, Copy, Debug, PartialEq, Default, VariantArray)]
pub enum Background {
    Forest,
    RedForest,
    Snow,
...
#[derive(Default)]
struct AnimationState { // TODO better name
    animation: Animation,
    frame: usize,
...
impl BattleData {
    fn from(battle: &Battle, depth: u16) -> Self {
        Self {
            depth,
            fighter: CombatantData { animation: AnimationState::from(Animation::FighterWalk), floating: Vec::new() },
            enemies: battle.enemies.iter().map(|c| CombatantData { animation: AnimationState::from(c.idle_animation()), floating: Vec::new() }).collect(),
        }
    }
    ...
    fn add_skill(&mut self, skill: &SkillStats) {
        match skill {
            ...
            SkillStats::Defend(_, def_stats) => {
                ...
                let color = if def_stats.healed > 0. {Color32::GREEN} else {Color32::LIGHT_GRAY};
