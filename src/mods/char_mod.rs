...
pub static SHIELD_START: ModType = ModType {
    ...
    prefix_name: "bulwark",
    roll_range: 30..=50,
    show_tooltip: |this, ui, roll| { tooltip!("Start combat with %roll% %range of your max health as shield"); },
    register: |hooks, _item, _equip, roll| {
...
pub static HEALTH_EX: ModType = ModType {
    ...
    prefix_name: "stout",
    roll_range: 0..=0,
    show_tooltip: |_this, ui, _roll| { tooltip!("Increase max health by 1000, if no other items give max health"); },
    register: |hooks, item, equip, _roll| {
        let has_health = equip.iter()
            .filter_map(|i| i.upgrade())
            .filter(|i| i.id != item.id)
            .any(|i| i.has_mod(HEALTH.id) > 0 || i.has_mod(HEALTH_EX.id) > 0);

        if !has_health {
            hooks.on_char(move |char| char.max_health += 1000.);
        }
    }
};
